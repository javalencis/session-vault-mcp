...
Diagnostics-first hardening and validation for session-vault installation flows.
...
- AND the command shape matches a valid install mode (e.g., `['npx', '-y', 'session-vault-serve']` or `['session-vault-serve']`)
...
- Real macOS error logs are still needed to diagnose specific platform failures; fixing speculative macOS root causes is explicitly OUT OF SCOPE for this change.
